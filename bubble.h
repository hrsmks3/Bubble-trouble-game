...
const int GRAVITY = 9.8;//downward acceleration of bubble
...
public:
    ...
    {
        // move the bubble
        vy = vy + GRAVITY*t;//Updation of vertical velocity
        double new_x = circle.getX() + vx*t;//Updation of X coordinate of particle
        double new_y = circle.getY() + vy*t + 0.5*GRAVITY*t*t;//Updation of Y coordinate of particle
        ...
        if ((vy < 0 && new_y < (0 + circle.getRadius())) // bounce along Y direction at upper border
            ||
            (vy > 0 && new_y > (WINDOW_Y - circle.getRadius() - 50))) // bounce along Y direction lower border
        {

            vy = -vy;
            new_y = circle.getY() + vx*t + 0.5*GRAVITY*t*t;
